...
description: Reads the public record for the names this desk holds and for a small number of contenders outside its universe, and writes one sourced memo. Every claim carries a URL. Produces no weight, no size, and no direction on a price. Use in the portfolio_watch template, after the analyst.
model: inherit
tools:
  - WebSearch
  - WebFetch
