...
**Verification gotchas, both learned the hard way.** (1) `nm target/debug/atlas` returns a FALSE 0/0/0 when `CARGO_TARGET_DIR` is redirected — the relative path does not resolve, so use an absolute one. (2) `cargo build` and `cargo build --no-default-features` write the SAME `target/debug/atlas` path and silently overwrite each other — snapshot each leg to its own filename (`atlas-armed`, `atlas-glass`) before comparing, or you are measuring one binary twice. (3) The symbol pattern must be the MANGLED path `5atlas3pty`, not `atlas.*pty`: the loose pattern matches `is_empty` and returns 20 on BOTH legs, which reads as a failed by-absence check when nothing is wrong. Verified counts on the two legs: `5atlas3pty` 108 armed / 0 glass, `portable_pty` 99/0, `vt100` 422/0.
